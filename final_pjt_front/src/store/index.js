...
export default new Vuex.Store({
  ...
  mutations: {
    ...
    SET_POPULAR_MOVIES(state) {
      ...
      state.popularMovies = state.movies.sort((a, b) => b.popularity - a.popularity).slice(start, start + 15)
    ...
    SET_LATEST_MOVIES(state) {
      const oneWeekAgo = new Date();
    oneWeekAgo.setDate(oneWeekAgo.getDate() - 7); // 1 week ago

    state.latestMovies = state.movies.filter(movie => {
      const movieReleaseDate = new Date(movie.release_date);
      return movieReleaseDate >= oneWeekAgo;
      });
    },
  },
  actions: {
    getMovies(context) {
      axios({
        ...
        .then((res) => {
          ...
          context.commit('SET_LATEST_MOVIES');
        })
        .catch((err) => {
          console.log(err)
        })
